...
- **EDT-5:** Authors can copy a complete question as JSON, replace another
  ...
  toast fixed to the viewport's bottom-right corner using the active
...
- **EDT-19:** Successful package creation, import, recent opening, current
  ...
  fixed to the viewport's bottom-right corner using the active application
  palette for two seconds. Canceled or failed actions and stale writes
  completed before newer edits show no success toast.
...
13. Clear a populated question from its trailing heading action, then use
    ...
    viewport's bottom-right corner in the active theme for two seconds and
    cancellation or failure shows none. Focus each editable field and confirm
    the shortcuts keep the browser's native field-level copy and paste behavior
    instead.
14. Create, import, reopen, edit, download, and delete packages and confirm
    ...
    viewport's bottom-right corner for two seconds. Cancel the desktop download,
    fail an action, and complete a stale autosave while a newer edit remains
    pending; confirm none shows a success toast.
...
18. Search beside an empty question heading with one and then two characters.
    ...
    separate status tag, while packages with readiness errors and no remarks
    show the has-errors tag.
