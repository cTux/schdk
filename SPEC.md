...
B126|2026-08-02|Fullscreen supplied the game surface height implicitly, so exiting fullscreen or restoring an active session left the fallback canvas collapsed inside the page layout|Make the active host page content fill its fixed viewport and hide page chrome as specified by `docs/specs/game-hosting.md` HST-9.
B127|2026-08-02|Splitting the timer duration from its displayed phases omitted the base question duration from the timer effect dependencies|Restore the complete effect dependency list; exhaustive-deps linting covers recurrence.
B128|2026-08-02|The comfortable and system font choices both resolved to Segoe UI on Windows while tooltips hardcoded it, so changing the preference could produce no visible UI change|Give the comfortable choice a distinct Trebuchet MS stack and make body and tooltip typography inherit the shared application font variable; `docs/specs/unified-shell.md` SHL-25 covers recurrence.
